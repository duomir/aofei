...
*
* Olli = additions from Oliver to specify the used Interface
*
*****************************************************************************/

const char *progname = "check_ping";
const char *copyright = "2015-2017";
const char *email = "461310110@qq.com";
...
char *interface = "\0";  /*Olli*/
char *r_command = NULL;  /*Olli*/
...
# endif
                ...
                /*Olli*/
                asprintf (&r_command,"%s -I %s", cmd, interface);
                /*Olli*/
                cmd = r_command;
                /*Olli*/
                }
...
process_arguments (int argc, char **argv)
{
        ...
        static struct option longopts[] = {
                ...
                {"interface", required_argument, 0 ,'I'}, /*Olli*/
                {0, 0, 0, 0}
        };
        ...
        while (1) {
                c = getopt_long (argc, argv, "VvhnL46t:c:w:H:p:I:", longopts, &option); /*Olli*/
                ...
                case 'w':
                        ...
                        break;
                case 'I': /* Interface used for Ping*/ /*Olli*/
                        interface = optarg; /*Olli*/
                        break; /*Olli*/
                }
        }
...
print_help (void)
{
  ...
  printf ("%s\n", _("Use ping to check connection statistics for a remote host."));
  ...
  printf ("    %s ", _("Specify the interface which is used for pinging (Requires iputils-ping)"));
  printf (" %s\n", "-L, --link");
  printf ("    %s\n", _("show HTML in the plugin output (obsoleted by urlize)"));

  printf (UT_TIMEOUT, DEFAULT_SOCKET_TIMEOUT);
  printf (UT_TIMEOUT);
  printf (DEFAULT_SOCKET_TIMEOUT);
...
print_usage (void)
{
 ...
 printf ("%s -H <host_address> -w <wrta>,<wpl>%% -c <crta>,<cpl>%%\n", progname);
  printf ("%s [-p packets] [-t timeout] [-4|-6] [-I interface] \n");
}
*/
void
print_help (void)
...
{
  printf ("%s\n", _("Usage:"));
        printf ("%s -I <src_address>-H <host_address> -w <wrta>,<wpl>%% -c <crta>,<cpl>%%\n", progname);
